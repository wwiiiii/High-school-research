...
void cRobot::init(int ID, double mas, DATA dat, int from, int to, coord start)
{
...
void cRobot::fConstForce(std::vector<coord>&CheckPoints){
	...
	{
		pnow = CheckPoints[RouteNowAt]; pnext = CheckPoints[RouteNowAt + 1];
		dist = sqrt( pow(pnow.x - pnext.x, 2) + pow(pnow.y - pnext.y , 2) );
		BasicForceX = 10 * (double)(pnext.x - pnow.x) / dist;
		BasicForceY = 10 * (double)(pnext.y - pnow.y) / dist;
		force_x += BasicForceX; force_y += BasicForceY;
		RouteNowAt++;
	}
	force_x += BasicForceX / 5000000000;
	force_y += BasicForceY / 5000000000;
}
